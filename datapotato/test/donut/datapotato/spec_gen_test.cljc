...
(deftest test-spec-gen-omit
  (testing "Ref not created and attr is not present when omitted"
    (let [gen (sg/ent-db-spec-gen-attr {:schema td/schema} {:todo-list [[:_ {:refs {:created-by-id ::dc/omit
  ...
  (testing "Ref is created when at least 1 field references it, but omitted attrs are still not present"
    (let [gen (sg/ent-db-spec-gen-attr {:schema td/schema} {:todo-list [[:_ {:refs {:updated-by-id ::dc/omit}}]]})]
      (is (td/submap? {:u0 {:user-name "Luigi"}} gen))
      (is (ids-present? gen))
      (is (ids-match? gen
                      {:tl0 {:created-by-id [:u0 :id]}}))
...
(defn insert-cycle
  ...
  (dc/ent-attr db ent-name sg/spec-gen-visit-key))
