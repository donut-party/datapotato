...
(def schema
  {:user            {:generate {:spec ::user}
                     :prefix   :u}
   :attachment      {:generate  {:spec ::attachment}
                     :relations {:created-by-id [:user :id]
                                 :updated-by-id [:user :id]
                                 :todo-id       [:todo :id]}
                     :prefix    :a}
   :todo            {:generate  {:spec       ::todo
                                 :overwrites {:todo-title "write unit tests"}}
                     :relations {:created-by-id [:user :id]
                                 :updated-by-id [:user :id]
                                 :todo-list-id  [:todo-list :id]}
                     :prefix    :t}
   :todo-list       {:generate  {:spec ::todo-list}
                     :relations {:created-by-id [:user :id]
                                 :updated-by-id [:user :id]}
                     :prefix    :tl}
   :todo-list-watch {:generate    {:spec ::todo-list-watch}
                     :relations   {:todo-list-id [:todo-list :id]
                                   :watcher-id   [:user :id]}
                     :constraints {:todo-list-id #{:uniq}}
                     :prefix      :tlw}
   :project         {:generate    {:spec ::project}
                     :relations   {:created-by-id [:user :id]
                                   :updated-by-id [:user :id]
...
(def polymorphic-schema
  {:topic-category {:generate {:spec ::topic-category}
                    :prefix   :tc}
   :topic          {:generate  {:spec ::topic}
                    :relations {:topic-category-id [:topic-category :id]}
                    :prefix    :t}
   :watch          {:generate  {:spec ::watch}
