...
(deftest test-spec-gen
  (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo-list [[1]]})]
...
(deftest test-spec-gen-nested
  (let [gen (dgm/generate-attr-map {:schema td/schema} {:project [[:_ {:refs {:todo-list-ids 3}}]]})]
...
(deftest test-spec-gen-manual-attr
  (testing "Manual attribute setting for non-reference field"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo [[:_ {:generate {:todo-title "pet the dog"}}]]})]
      (is (td/submap? {:u0 {:username "Luigi"}
  ...
  (testing "Manual attribute setting for reference field"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo [[:_ {:generate {:created-by-id 1}}]]})]
      (is (td/submap? {:u0 {:username "Luigi"}
                       :t0 {:created-by-id 1}}
                      gen))
      (is (ids-present? gen))
...
(deftest test-spec-gen-omit
  (testing "Ref not created and attr is not present when omitted"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo-list [[:_ {:refs {:created-by-id ::dd/omit
  ...
  (testing "Ref is created when at least 1 field references it, but omitted attrs are still not present"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo-list [[:_ {:refs {:updated-by-id ::dd/omit}}]]})]
      (is (td/submap? {:u0 {:username "Luigi"}} gen))
      (is (ids-present? gen))
      (is (ids-match? gen
                      {:tl0 {:created-by-id [:u0 :id]}}))
...
(deftest overwriting
  (testing "Overwriting generated value with query map"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo-list [[:_ {:generate {:updated-by-id 42}}]]})]
      (is (ids-present? gen))
      (is (= 42 (-> gen :tl0 :updated-by-id)))))

  (testing "Overwriting generated value with query fn"
    (let [gen (dgm/generate-attr-map {:schema td/schema} {:todo-list [[:_ {:generate #(assoc % :updated-by-id :foo)}]]})]
      (is (ids-present? gen))
...
(deftest test-insert-gen-data
  (-> (dgm/generate {:schema td/schema} {:todo [[1]]})
...
(deftest inserts-novel-data
  ...
  todo that references the same todo list and user"
    (let [db1 (-> (dgm/generate {:schema td/schema} {:todo [[1]]})
                  (dd/visit-ents-once :inserted-data insert))]
...
(deftest handle-cycles-with-constraints-and-reordering
  (testing "todo-list is inserted before todo because todo requires todo-list"
    (-> (dgm/generate {:schema td/cycle-schema} {:todo [[1]]})
